...
use crate::{
    ...
    normalize::normalize_answer_updated_event,
    protocol_map::OracleProtocolMap,
};
...
impl EvmChainAdapter {
    ...
    async fn fetch_logs_in_range(&mut self, from: U64, to: U64) -> Result<LogBatch> {
        ...
        let mut last_provider_index = self.active_provider_index;
        ...
        for chunk in all_addresses.chunks(self.filter_chunk_size.max(1)) {
            ...
            last_provider_index = batch.provider_index;
            logs.extend(batch.logs);
        }

        Ok(LogBatch {
            logs,
            provider_index: last_provider_index,
        })
    }
    ...
    fn decode_logs(
        ...
        provider_index: usize,
    ) -> Vec<common_types::NormalizedEvent> {
        ...
        for log in logs {
            let Some(bindings) = self.oracle_protocol_map.get(&log.address) else {
                continue;
            };

            let Some(raw_answer) = decode_answer_updated_raw(&log) else {
                continue;
            };
            ...
            for (binding_index, binding) in bindings.iter().enumerate() {
                ...
                if dedupe_on_event_key && !self.seen_event_keys.insert(event.event_key.clone()) {
                    continue;
                }
    ...
    async fn fetch_logs_with_failover(&mut self, filter: &Filter) -> Result<LogBatch> {
        let provider_count = self.providers.len();
        for offset in 0..provider_count {
            let idx = (self.active_provider_index + offset) % provider_count;
            let provider = self.providers[idx].provider.clone();
...
impl ChainAdapter for EvmChainAdapter {
    async fn next_events(&mut self) -> Result<Vec<common_types::NormalizedEvent>> {
        if self.oracle_protocol_map.is_empty() {
            warn!(
                chain = self.chain_slug,
                "evm adapter has no oracle addresses configured"
            );
            return Ok(vec![]);
        }
        ...
        if batch.logs.is_empty() {
            info!(
                chain = self.chain_slug,
                "evm adapter found no oracle updates in lookback window"
            );
            return Ok(vec![]);
        }
    ...
    async fn backfill_range(
        ...
        from_block: u64,
        to_block: u64,
    ) -> Result<Vec<common_types::NormalizedEvent>> {
