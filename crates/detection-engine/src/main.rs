...
use chrono::Utc;
use event_schema::{AlertEvent, NormalizedEvent, ProtocolCategory};
use common::{AlertSink, ChainAdapter, RiskScorer, RuleEvaluator, ShutdownSignal};
use dotenv::dotenv;
use ethers::types::Address;
use state_manager::RedisStreamPublisher;
use state_manager::PostgresRepository;
use feature_builder::{LiveReferencePriceProvider, ReferencePriceProvider};
use risk_scorer::DeterministicRiskScorer;
use detection_engine::{RuleDefinition, RuleEngine};
use serde::de::DeserializeOwned;
use feature_builder::enrich_with_trace;
...
async fn main() -> Result<()> {
    ...
    let emit_alerts = std::env::var("DETECTION_ENGINE_EMIT_ALERTS")
        .ok()
...
async fn run_stream_mode(
    ...
    let batch_size = std::env::var("DETECTION_ENGINE_STREAM_BATCH_SIZE")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(DEFAULT_STREAM_BATCH_SIZE);
    let block_ms = std::env::var("DETECTION_ENGINE_STREAM_BLOCK_MS")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(DEFAULT_STREAM_BLOCK_MS);
    let run_once = std::env::var("DETECTION_ENGINE_STREAM_RUN_ONCE")
        .ok()
        .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
        .unwrap_or(false);
    let use_consumer_group = std::env::var("DETECTION_ENGINE_STREAM_USE_CONSUMER_GROUP")
        .ok()
        .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
        .unwrap_or(true);

    if use_consumer_group {
        let group = std::env::var("DETECTION_ENGINE_STREAM_GROUP")
            .unwrap_or_else(|_| "detection-engine-workers".to_string());
        let consumer = std::env::var("DETECTION_ENGINE_STREAM_CONSUMER")
        ...
        loop {
            ...
            if shutdown.is_shutdown_requested() {
                info!("shutdown signal received; stopping detection-engine gracefully");
                return Ok(());
            }
    ...
    let mut last_id =
        std::env::var("DETECTION_ENGINE_STREAM_START_ID").unwrap_or_else(|_| "0-0".to_string());
...
fn alert_from_detection(detection: &event_schema::DetectionResult) -> AlertEvent {
    AlertEvent {
...
fn should_use_stream_mode() -> bool {
    std::env::var("DETECTION_ENGINE_USE_STREAM_MODE")
        .ok()
        .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
        .unwrap_or(true)
}
...
async fn init_repository() -> Option<PostgresRepository> {
    ...
    match PostgresRepository::from_database_url(&database_url).await {
        Ok(repo) => {
            info!("postgres state_manager enabled");
            Some(repo)
        }
        Err(err) => {
