...
use chrono::Utc;
use common_types::{NormalizedEvent, ReorgNotice};
...
const DEFAULT_INDEXER_POLL_INTERVAL_SECS: u64 = 5;
const DEFAULT_INDEXER_BLOCK_BATCH_SIZE: u64 = 8;
const DEFAULT_INDEXER_MAX_RETRIES: usize = 5;
const DEFAULT_INDEXER_RETRY_BACKOFF_MS: u64 = 500;
const INDEXER_CHAIN_LOCK_NAMESPACE: i32 = 42_017;
...
impl ChainTrackerState {
    fn observe_event(&mut self, event: &common_types::NormalizedEvent) -> Option<ReorgNotice> {
    ...
    async fn try_acquire_chain_lock(&self, chain_slug: &str) -> Result<bool> {
        let row = self
            ...
            .query_one(
                ...
                "#,
                &[&INDEXER_CHAIN_LOCK_NAMESPACE, &chain_slug],
            )
            .await?;
...
async fn main() -> Result<()> {
    ...
    if adapters.is_empty() {
        warn!("no chain adapters available; exiting indexer-worker");
        return Ok(());
    }
    ...
    let Some(stream) = init_stream_publisher().await else {
        warn!("REDIS_URL not set or unavailable; indexer-worker requires Redis Streams");
        return Ok(());
    };
    ...
    let run_once = std::env::var("INDEXER_RUN_ONCE")
        .ok()
        .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
        .unwrap_or(false);
    let configured_batch_size = std::env::var("INDEXER_BLOCK_BATCH_SIZE")
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
        .unwrap_or(DEFAULT_INDEXER_BLOCK_BATCH_SIZE)
        .max(1);
    let max_retries = std::env::var("INDEXER_MAX_RETRIES")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(DEFAULT_INDEXER_MAX_RETRIES)
        .max(1);
    let retry_backoff_ms = std::env::var("INDEXER_RETRY_BACKOFF_MS")
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
        .unwrap_or(DEFAULT_INDEXER_RETRY_BACKOFF_MS)
        .max(1);
    ...
    loop {
            ...
            if let Some(store) = state_store.as_ref() {
                ...
                if previous != Some(has_lock) {
                    if has_lock {
                        info!(
                            chain_slug = %runtime.chain_slug,
                            lock_namespace = INDEXER_CHAIN_LOCK_NAMESPACE,
        ...
        if run_once {
            info!(
                emitted_count,
                "INDEXER_RUN_ONCE=true; stopping after first pass"
            );
            break;
        }
        ...
        let protocol_map = build_oracle_protocol_map(oracle_addresses.into_iter().map(|address| {
            (
                ...
                ProtocolBinding {
                    ...
                    oracle_decimals,
                    protocol_category: common_types::ProtocolCategory::Lending,
                },
            )
        }));
            ...
            Err(err) => {
                ...
                Box::new(
                    EvmMockAdapter::single_protocol(
                        ...
                        1,
                        eth_protocol,
                        common_types::ProtocolCategory::Lending,
                        3280.0,
                        3120.0,
                    )
                    .with_confirmation_depth(eth_confirmation_depth),
        ...
        if oracle_addresses.is_empty() {
            Box::new(
                EvmMockAdapter::single_protocol(
                    ...
                    8453,
                    base_protocol,
                    common_types::ProtocolCategory::PerpDex,
                    3010.0,
                    3000.0,
                )
                ...
                build_oracle_protocol_map(oracle_addresses.into_iter().map(|address| {
                    (
                        ...
                        ProtocolBinding {
                            ...
                            oracle_decimals,
                            protocol_category: common_types::ProtocolCategory::PerpDex,
                        },
                    )
                }));
