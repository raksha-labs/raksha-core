...
use super::{empty_normalized, observed_at, parse_i64, parse_ts_from_path, source_event_id, ParsedFeedEvent, ParserInput};
...
pub(super) fn parse(input: &ParserInput<'_>, payload: &Value) -> Result<ParsedFeedEvent, String> {
    ...
    let block_number = parse_i64(payload.get("blockNumber").or_else(|| payload.get("block_number")));
    ...
    let topic0 = payload
        ...
        .and_then(Value::as_str)
        .map(ToString::to_string)
        .or_else(|| payload.get("topic0").and_then(Value::as_str).map(ToString::to_string));
    ...
    if let Some(obj) = normalized.as_object_mut() {
        ...
        obj.insert("data".to_string(), payload.get("data").cloned().unwrap_or(Value::Null));
    }
