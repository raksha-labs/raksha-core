...
use anyhow::Result;
use chrono::Utc;
use common_types::{AlertEvent, DetectionResult, LifecycleState};
...
async fn main() -> Result<()> {
    ...
    let Some(stream) = init_stream_publisher().await else {
        warn!("REDIS_URL not set or unavailable; alert-orchestrator requires Redis Streams");
        return Ok(());
    };
    ...
    let batch_size = std::env::var("ALERT_ORCH_STREAM_BATCH_SIZE")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(DEFAULT_BATCH_SIZE);
    let block_ms = std::env::var("ALERT_ORCH_STREAM_BLOCK_MS")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(DEFAULT_BLOCK_MS);
    let run_once = std::env::var("ALERT_ORCH_RUN_ONCE")
        .ok()
        .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
        .unwrap_or(false);
    let use_consumer_group = std::env::var("ALERT_ORCH_USE_CONSUMER_GROUP")
        .ok()
        .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
        .unwrap_or(false);

    let mut detections_last_id =
        std::env::var("ALERT_ORCH_DETECTIONS_START_ID").unwrap_or_else(|_| "0-0".to_string());
    let mut finality_last_id =
        std::env::var("ALERT_ORCH_FINALITY_START_ID").unwrap_or_else(|_| "0-0".to_string());
    let stream_group = std::env::var("ALERT_ORCH_STREAM_GROUP")
        .unwrap_or_else(|_| "alert-orchestrator-workers".to_string());
    let stream_consumer = std::env::var("ALERT_ORCH_STREAM_CONSUMER")
        .unwrap_or_else(|_| default_consumer_name("alert-orch"));
    ...
    if use_consumer_group {
        ...
        info!(
            ...
            "alert-orchestrator stream consumer-group mode enabled"
        );
    }

    let mut alerts_by_event_key: HashMap<String, AlertEvent> = HashMap::new();
    info!("alert-orchestrator started");
    ...
    loop {
        ...
        if run_once {
            info!(
                processed,
                "ALERT_ORCH_RUN_ONCE=true; stopping after one loop"
            );
            break;
        }
